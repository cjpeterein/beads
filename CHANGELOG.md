...
- **`BEADS_SESSION_ID` is the primary session env var; `CLAUDE_SESSION_ID` is a deprecated alias.** Session attribution for close-time audit (`closed_by_session`) now resolves through a shared `--session` flag > `BEADS_SESSION_ID` > `CLAUDE_SESSION_ID` chain, mirroring the existing `BEADS_ACTOR` > `BD_ACTOR` deprecation. `bd supersede` and `bd duplicate` gained a `--session` flag for parity with `bd close` / `bd update`, and their close path no longer reads `CLAUDE_SESSION_ID` directly — the vendor-specific env read is consolidated into one helper. `CLAUDE_SESSION_ID` continues to work. (bd-p50l)
